...
@eqx.filter_custom_vjp
def _loop_checkpointed_reversible(
    y__args__terms, *, self, throw, max_steps, init_state, **kwargs
):
    del throw
    y, args, terms = y__args__terms
    init_state = eqx.tree_at(lambda s: s.y, init_state, y)
...
@_loop_checkpointed_reversible.def_fwd
def _loop_checkpointed_reversible_fwd(perturbed, y__args__terms, **kwargs):
    del perturbed
    final_state, aux_stats = _loop_checkpointed_reversible(y__args__terms, **kwargs)
    init_ts = final_state.reversible_init_ts
    ts = final_state.reversible_ts
    ts_final_index = final_state.reversible_save_index
    y1 = final_state.y
    save_state = final_state.save_state
    solver_state = final_state.solver_state
    c_ts = final_state.reversible_checkpointed_ts
    c_ys = final_state.reversible_checkpointed_ys
    c_zs = final_state.reversible_checkpointed_zs
    return (final_state, aux_stats), (
        init_ts,
        ts,
        ts_final_index,
...
@_loop_checkpointed_reversible.def_bwd
def _loop_reversible_bwd(
    residuals,
    grad_final_state__aux_stats,
    perturbed,
    ...
    **kwargs,
    ...
    del perturbed, self, init_state, kwargs
    init_ts, ts, ts_final_index, y1, save_state, solver_state, c_ts, c_ys, c_zs = (
        residuals
    )
    del residuals
...
class CheckpointedReversibleAdjoint(AbstractAdjoint):
    """Backpropagate through [`diffrax.diffeqsolve`][] when using a reversible solver
    [`diffrax.AbstractReversibleSolver`][].
    ...
    def loop(
        ...
        **kwargs,
        ...
        final_state, aux_stats = _loop_checkpointed_reversible(
            (y, args, terms),
            self=self,
            saveat=saveat,
            max_steps=max_steps,
